# Padrão da instrução MIPS 32 bits 
# op      rs     rt     rd     sa     fu
# 000000  00000  00000  00000  00000  000000
# 6b      5b     5b     5b     5b     6b
#                      (------imediato------)
def instrucoes(nome):
  instrucoes = {
    "add": {
      "instrucao": "000000std00000100000",
      "padrao": "r",
      "imediato": False
    },
    "addi": {
      "instrucao": "001000sti",
      "padrao": "i",
      "imediato": True
    },
    "addiu": {
      "instrucao": "001001sti",
      "padrao": "i",
      "imediato": True
    },
    "addu": {
      "instrucao": "000000std00000100001",
      "padrao": "r",
      "imediato": False
    },
    "and": {
      "instrucao": "000000std00000100100",
      "padrao": "r",
      "imediato": False
    }
  }
  if nome in instrucoes:
    return(f"{nome} = {instrucoes[nome]['instrucao']}")
  else:
    return(f"Instrução '{nome}' inválida")

def registradores(registrador):
  registradores = {
    "zero": 0,
    "at": 1,
    "v0": 2,
    "v1": 3,
    "a0": 4,
    "a1": 5,
    "a2": 6,
    "a3": 7,
    "t0": 8,
    "t1": 9,
    "t2": 10,
    "t3": 11,
    "t4": 12,
    "t5": 13,
    "t6": 14,
    "t7": 15,
    "s0": 16,
    "s1": 17,
    "s2": 18,
    "s3": 19,
    "s4": 20,
    "s5": 21,
    "s6": 22,
    "s7": 23,
    "t8": 24,
    "t9": 25,
    "k0": 26,
    "k1": 27,
    "gp": 28,
    "sp": 29,
    "fp": 30,
    "ra": 31
  }
  if registrador in registradores:
    return registradores[registrador]
  else:
    return f"Registrador {registrador} inválido"